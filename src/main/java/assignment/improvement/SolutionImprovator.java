...
public class SolutionImprovator implements ISolutionImprovator {
    ...
    public void iterate() {
        ...
        while((model.estimateNumberOfConflictOfExam(nextTimeSlot, selectedExam) != 0 || (nextTimeSlot == actualTimeSlot)) && maxAttempt < model.getTimeslotsNumber()){ //TODO: AND NEXTTIMESLOT != ACTUAL
            //Nel caso ci siano conflitti e lo spostamento non sia accettabile
            nextTimeSlot = random.nextInt(model.getTimeslotsNumber());
            maxAttempt++;
        }
        ...
        if(model.getScoreOfAMove(selectedExam, actualTimeSlot, nextTimeSlot) < model.getActualScore()){
